...
class EmailVerificationController {
    ...
    public function sendVerificationCode($params) {
        ...
        // 如果是注册验证，检查邮箱是否已存在
        if ($type === 'register') {
            $check_sql = "SELECT COUNT(*) FROM " . "qwq_user WHERE email = :email";
        ...
        if ($type === 'reset_password') {
            $check_sql = "SELECT COUNT(*) FROM " . "qwq_user WHERE email = :email AND status = 1";
            $stmt = $this->db->prepare($check_sql);
            $stmt->bindParam(':email', $email);
            $stmt->execute();
        ...
        try {
            // 检查是否已有验证码记录
            $check_code_sql = "SELECT id FROM " . "qwq_verification_code WHERE email = :email AND type = :type";
