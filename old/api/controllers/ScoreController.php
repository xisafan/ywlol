...
class ScoreController {
    ...
    public function getAverageScore($params) {
        ...
        $stmt = $this->pdo->prepare("SELECT AVG(score) as avg_score FROM qwq_score WHERE vod_id = ?");
        $stmt->execute([$vod_id]);
    ...
    public function getScoreDetails($params) {
        ...
        // 获取所有评分详情
        $stmt = $this->pdo->prepare("SELECT username, score, comment, likes FROM qwq_score WHERE vod_id = ? ORDER BY id DESC");
        $stmt->execute([$vod_id]);
    ...
    public function addScore($params) {
        ...
        $stmt = $this->pdo->prepare("INSERT INTO qwq_score (vod_id, username, score, comment, likes) VALUES (?, ?, ?, ?, ?)");
